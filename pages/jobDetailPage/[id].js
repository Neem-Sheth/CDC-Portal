...
export default function JobDetailPage() {
  ...
  const applyForJob = async () => {
    ...
    try {
      // Replace this with the actual user email retrieved from authentication
      const username = localStorage.getItem("username"); 
      console.log(username);
  
      const res = await fetch("/api/sendJobApplicationEmail", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          username,
          job, // Pass the job details to the backend
        }),
      });
  
      if (!res.ok) throw new Error("Failed to send application email");
  
      setHasApplied(true); // Update the state to reflect successful application
      alert("Application successful! Details have been sent to your email.");
    } catch (error) {
      console.error(error);
      alert(error.message);
    }
